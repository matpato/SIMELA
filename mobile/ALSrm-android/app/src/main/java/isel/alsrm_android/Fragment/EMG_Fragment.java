...
public class EMG_Fragment extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if(fragmentRootContainer == null) {
            ...
            if (chart == null)
                chart = new Chart((LineChart) fragmentRootContainer.findViewById(R.id.chart), EMG+" - "+ Utils.muscleAbbreviation(muscle), 1.65f, -1.65f);
                ...
                if (!stop) {
                    if (!run) {
                        if (MainFragment.bluetoothDevice != null) {
                            ...
                            intentService.putExtra(ManageConnectedSocket.EXAM_TYPE, EMG);
                            intentService.putExtra(MainActivity.MUSCLE, muscle);
                            intentService.putExtra(ManageConnectedSocket.ANALOG_CHANNELS,  new int[]{0});
    ...
    public void onResume() {
        ...
        if(muscle == AlsrmSchema.AnteriorTibialis)
            intentFilter = new IntentFilter(ManageConnectedSocket.ACTION_IntentService_EMG_AT);
        if(muscle == AlsrmSchema.FlexorCarpiRadialis)
            intentFilter = new IntentFilter(ManageConnectedSocket.ACTION_IntentService_EMG_FCR);
        if(muscle == AlsrmSchema.SternocleidoMastoideus)
            intentFilter = new IntentFilter(ManageConnectedSocket.ACTION_IntentService_EMG_SCM);
    ...
    private class BroadcastReceiverData extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(intent.getBooleanExtra(ManageConnectedSocket.EXAM_COMPLETED, false)){
                ...
                chart.addEntry(intent.getIntExtra(ManageConnectedSocket.X_KEY_OUT, 0), intent.getDoubleArrayExtra(ManageConnectedSocket.DATA_KEY_OUT)[0]);
